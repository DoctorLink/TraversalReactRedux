...
const HealthAgeDial = ({ age, healthAge, minimumHealthAge }) => {
    ...
    return (
        <svg viewBox={[0, 0, width, height]}>
            <DialArc cx={centre.x} cy={centre.y} radius={arcRadius} startAngle={-90} endAngle={90} strokeWidth={arcStrokeWidth} />
            <DialPointer cx={centre.x} cy={centre.y} radius={pointerRadius} width={pointerWidth} height={pointerHeight} angle={angle} />
            <text x={centre.x} y={centre.y * 0.75} textAnchor="middle">{healthAge}</text>
        </svg>
    );
}
